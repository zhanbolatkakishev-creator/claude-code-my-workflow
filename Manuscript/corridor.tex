...
Kazakhstan retains, and ask whether it induced investment. Exports to Russia in 29 product
lines break discontinuously in May 2022 and rise roughly tenfold (a selection-free,
externally compiled priority list not chosen on this outcome rises 3.5$\times$ over the same
window); the break date survives an
HAC correction, but the basket's difference-in-differences is not separable from its own
selection rule, and a 50-line externally compiled priority list corroborates the break date
and, on its 26 non-overlapping lines against a purged control, a smaller-magnitude version of
the effect, so identification rests on the break dates, raw levels, and a
neighbour comparison rather than on the cross-sectional regression alone. Propagating a
wholesale-and-freight margin (6--14\%, a band we choose rather than estimate) through the
input--output table implies domestic value added of 5--11 cents per rerouted dollar, against
about three-quarters for a produced dollar, a reading that holds for any margin under about a
fifth. Across three commercial deal databases we find no investment response in the sectors the
flow passes through, though four post-2022 years rule out only a large one. We read the null
through three gates --- market access, irreversibility and capital-market institutions.
An extreme irreversibility configuration or a capital market that cannot fund the projects
that pass would each drive the response toward zero on their own; an open market-access
gate does so only jointly with a small transformation margin, not by itself. The case
identifies an open market-access gate, created by a customs union that makes trade a close
substitute for production, together with suggestive (not dispositive) evidence for that small
margin, and finds that the least capital-constrained investor in the economy did not build
either; whether irreversibility independently binds cannot be settled here. Kazakhstan operates as a corridor, not a factory.
...
lines rose roughly tenfold (3.5$\times$ on a selection-free, externally compiled priority list
not chosen on this outcome; Section~\ref{sec:shock}). The routes, the demand and the products are all observable in the
...
doubled from 2019--2021 to 2022--2024. The outbound aggregate breaks sharply in 2022m5 (monthly
sup-$F$ of 561, or 326 under a Newey--West correction, with an unchanged confidence interval);
the inbound flow breaks in the same month but more weakly, and not at all in the noisier annual
series, where a large pre-existing China flow dominates. Armenia and T\"urkiye show the same
2022 break, and the Kyrgyz Republic and Georgia show elevated exports from 2022 on a noisier
series, all with no comparable domestic reform (Appendix~\ref{app:breaks}). A difference-in-differences against
control lines gives $\gamma = 2.44$ on the data-driven basket, but a selection-rule-matched
permutation places the estimate inside its own null distribution, so we do not read that
coefficient as an identified magnitude. The externally compiled priority list breaks at the
same date, and offers partial corroboration at a smaller magnitude: its own level rise is a
third as large (3.5$\times$ against the surge basket's 11.6$\times$), and the 26 priority-list
lines that are not already in the surge basket -- estimated against a control purged of the
surge basket itself, rather than the full panel -- show a smaller but statistically
distinguishable rise on the outbound flow to Russia ($\gamma = 1.94$, wild-cluster bootstrap
$p = 0.035$; own level rise 2.4$\times$; pre-trend joint Wald $F = 0.75$, $p = 0.528$), though
not on the inbound flow ($\gamma = 1.12$,
$p = 0.238$). The priority list's larger, shared-line coefficient is not an artifact of the 24
lines it shares with the surge basket alone, but its magnitude is not independent of the
surge basket either: the residual lines move in the same direction at a smaller magnitude.
A placebo on the largest civilian lines runs the other way, with no
overlap between the lines it fake-treats and the surge basket.
...
on the incremental flow is well under 1\% of customs revenue, and the gross reoriented flow is
0.2--0.3\% of GDP a year.

\emph{Third, we ask whether the shock induced investment.} We find no response the data can
...
production), which is close to sufficient for a null only jointly with a small transformation
margin, not on its own (Section~\ref{sec:framework}), and which also means Kazakhstan forgoes
the technology transfer a rules-of-origin regime would have forced \citep{desouza2024diffusion}. The case
is also inconsistent with capital-market institutions as the binding constraint for this particular null:
...
retains from a post-2022 trade reallocation and of whether it induced investment, extending a
literature that establishes the fact and scale of the reallocation to what the intermediary
retains from it, and to 2025. It also connects to \citet{bowncrowley2007}'s finding that
import-restricting trade policy deflects a foreign country's exports into third markets: the
present case takes that deflected flow as its starting point and asks what the receiving
economy does with it. It shows that a large,
...
uncertainty and irreversibility \citep{dixitpindyck1994,handleylimao2015,handleylimao2017}, and
...
production, engaging the alternative that an export-platform FDI response should instead
predict \citep{brainard1997,helpmanmelitzyeaple2004,ekholmforslidmarkusen2007}. As Section~\ref{sec:why} sets out, the case establishes an open market-access gate
and finds evidence inconsistent with the institutional gate as the binding constraint for this
null, and leaves the
...
Section~\ref{sec:moderators} turns to the moderators and external validity;
Section~\ref{sec:where} identifies where investment would raise value capture;
Section~\ref{sec:policy} draws the policy implications; and
...
as intra-union. The union aligned Kazakhstan's external tariff schedule with Russia's, largely
at the Russian duty levels prevailing at the time, and measurably shifted the sourcing of its
imports away from China and toward within-union partners, though the estimated magnitudes are modest
...
basket that moves far less, and the same disruption evident in Armenia and T\"urkiye by 2022
(the Kyrgyz Republic and Georgia elevated on noisier series; Appendix~\ref{app:breaks}) under no
comparable reform.
...
around the shock's size and duration and decreasing in $\rho$ \citep{dixitpindyck1994}. The
policy-contingent uncertainty that raises $\Omega$ here is the same channel
\citet{handleylimao2015,handleylimao2017} formalise and estimate for trade-policy uncertainty
depressing export entry and investment; \eqref{eq:build} adds the re-export outside option
$V_T$ to that channel, which is the addition the market-access gate below turns on. $I$ and
$\Omega$ are the terms this literature owns: sunk market-entry costs generate the same
hysteresis in export participation \citep{robertstybout1997}, and their structural
estimates put entry costs an order of magnitude above per-period profits and highly
heterogeneous across producers, which is why credible persistence, not merely large expected
demand, is what moves the entry margin \citep{dasrobertstybout2007}. Four
observations follow.
...
extreme, no entrant builds and the investment response $R$ is near zero, regardless of the
other conditions. This is a statement about \emph{whether anyone builds}, and it does not
depend on who the entrant is or how deep the capital market is.
...
earns), building is dominated before $\Omega$ is added. We do not identify $\mu_P$ or $\mu_T$
directly; two pieces of evidence are suggestive, not dispositive, that $\mu_P \lesssim \mu_T$
for this sector. First, its domestic value-added multiplier ($0.69$; Section~\ref{sec:where})
is a share of gross output accruing domestically, not a per-unit margin, so it bears on
$\mu_P$ only loosely: a low multiplier is consistent with a low transformation margin but does
not measure it. Second, the sector's near-zero domestic base ($\approx$ \$230m of electronics
output) is itself the outcome the paper is trying to explain, so citing it as evidence for
$\mu_P \lesssim \mu_T$ risks circularity --- an absent industry is equally what a customs union
with no comparative advantage in the sector would produce, independent of any margin
comparison. We return to this alternative reading, and what would distinguish it, in
observation (iii).
...
what makes trade a viable substitute for production.

This is not the only prediction a customs union makes, however, and the case for the gate
having bound here is joint, not stand-alone. The export-platform FDI literature
\citep{brainard1997,helpmanmelitzyeaple2004,ekholmforslidmarkusen2007} shows that lower trade
costs with a destination can pull \emph{production} toward the member that serves it
tariff-free just as readily as they enable pure re-export: a customs union lowers the cost of
shipping a finished good onward from a local plant exactly as it lowers the cost of re-export,
so $\mu_P$ and $V_T$ can rise together, and \eqref{eq:build} on its own does not say which
effect dominates. An open market-access gate drives $R$ toward zero only jointly with a small
transformation margin ($\mu_P \lesssim \mu_T$); membership in a customs union alone does not
deliver that combination, which is why observation (ii) treats the margin comparison as
suggestive rather than settled. What would distinguish the two readings is direct evidence on
$\mu_P$: the transformation margin actually available to a Kazakh producer in a
customs-union-facilitated line elsewhere in the economy, or a comparison with an economy at a
similar income level that lacks a customs union with its main trading partner. We do not have
that comparison. The case here identifies an open market-access gate, but it cannot rule out
that Kazakhstan's absence of an electronics industry is equally consistent with the country
simply lacking a comparative advantage in the sector, customs union or not.

The same logic runs through the
...
The three conditions can be summarised as a product, though only two of the four observations
above identify a factor that alone drives the investment response to zero regardless of the
others: an extreme irreversibility configuration, or a capital market that cannot fund the
projects that pass. The market-access gate is different, as observation (iii) argues: an open
gate (transformation not required, so $V_T$ is large) drives $R$ toward zero only jointly with
a small transformation margin, not on its own. Writing
\begin{equation}
\begin{aligned}
...
\label{eq:gates}
\end{equation}
summarises the case's argument; it is not a result derived from \eqref{eq:build} (whose
threshold is on a sum, with the three considerations interacting inside a single indicator
rather than entering as separable multiplicative terms). We do not claim, and do not need,
that the three conditions are separately \emph{necessary}: each of the arguments made below
--- irreversibility, market access taken jointly with the transformation-margin evidence, and
institutions --- is argued for Kazakhstan on its own terms in Sections~\ref{sec:shock}--%
\ref{sec:why}, rather than derived from \eqref{eq:gates}. Those sections show that for
Kazakhstan the market-access gate is open and, jointly with the sector's low transformation
margin, close to sufficient for $R \approx 0$ (subject to the alternative reading noted above);
that the state investment corporation, which faces neither an external-finance nor an exit
constraint, did not build for the reallocation either, which is inconsistent with capital-market
institutions
as the binding constraint for this null; and that whether the irreversibility gate
independently binds cannot be settled. Section~\ref{sec:moderators} and Section~\ref{sec:where}
show that the institutional gate does explain a separate fact: the durable adjacent
opportunities the shock reveals go unfinanced.
...
Appendix~\ref{app:products} (Table~\ref{tab:appb}). Because the basket is selected on the
...
taken from \citet{qicaifcifc2026pe}, which reports it in aggregate (about 98 investment
records, roughly \$2.2bn, 2015--2025), by sector and year, and names a handful of individual
investments in its case studies; no project-level QIC register is public, and the paper uses
only the report's aggregate, sector-timing and named-investment evidence.
...
so the 2025 West + China total is the Western component only. The priority-list row is the
externally compiled 50-HS6 list (not selected on this outcome), shown as a selection-free
comparator to the surge basket's "roughly tenfold" rise.}
...
\midrule
West + China $\to$ KZ (mirror) & 394 & 427 & 483 & 470 & \textbf{837} & 1{,}373 & 2{,}360 & 363$^{\dagger}$ \\
\quad \emph{of which Western}   & 162 & 162 & 168 & 184 & \textbf{337} & 444 & 419 & 363 \\
Kazakhstan $\to$ Russia        &  17 &  12 &   7 &   8 & \textbf{128} & 145 & 119 & 133 \\
\emph{Priority list, overlapping (50 HS6)} & 82 & 89 & 59 & 61 & \textbf{306} & 293 & 229 & 187 \\
\bottomrule
\end{tabular}

\vspace{2pt}
\footnotesize $^{\dagger}$ 2025 West + China is the Western component only; China has not yet
reported. Priority-list pre-2022 mean \$72.7m, post-2022 mean \$253.7m, ratio 3.5$\times$,
against the surge basket's 11.6$\times$ (Section~\ref{sec:shock}).
\end{table}

\subsection{Timing and the event study}

Monthly, the outbound series (Kazakhstan's exports to Russia) breaks sharply in 2022m5
(Bai--Perron \citep{baiperron1998}, 95\% CI 2022m4--2022m6; sup-$F$ \citep{andrews1993} of 561 under a homoskedastic covariance, against
critical values in the low tens), following the March-2022 sanctions with a short lag and
preceding every political-calendar date. Because the series is persistent, a homoskedastic
covariance can overstate the sup-$F$ and understate the confidence interval; under a
Newey--West \citep{neweywest1987} correction the sup-$F$ falls to 326 but the 95\% CI is unchanged (2022m4--2022m6),
so the timing argument does not depend on the choice of covariance estimator. The inbound
series (West + China) breaks in the same
month (95\% CI 2022m4--2022m6, HAC sup-$F$ 15.5 against 24.1 homoskedastic, CI unchanged), with
a second, smaller break in late 2023 as trade-monitoring
measures tightened, and the Western component alone breaks in 2022m5 more sharply (95\% CI
widens by one month to 2022m3--2022m6 under the HAC correction). On the
...
referendum, that the response is confined to a narrow product set, and that Armenia and
T\"urkiye show the same 2022 break, with the Kyrgyz Republic and Georgia also elevated from
2022 on noisier series, and none with a comparable domestic reform (Appendix~\ref{app:breaks})
...
mirrored exports to Kazakhstan, red = Kazakhstan's exports to Russia. 2024 omitted
(Kazakhstan's monthly reporting to Comtrade is suspended after 2024m2). Dashed line:
March 2022. Source: UN Comtrade.}
\label{fig:monthly}
\end{figure}
...
and rare after (23\% of surge-basket cells are zero in 2018--2021 against 3\% in 2022--2025), so
we report the asinh specification alongside a Poisson (PPML) estimate in levels
\citep{silvatenreyro2006}, which handles
the zeros without a transformation and weights by size. $y$ is measured in raw USD, so with a
substantial zero-and-near-zero pre-period mass the asinh coefficient does not have a single
clean percentage-change reading across cells \citep{bellemarewichman2020}; the two specifications agree on sign and
significance but not on implied magnitude (the asinh point estimate corresponds to a level
effect several times larger than PPML's $3.5\times$), consistent with the general warning
against reading log-like transformations of a variable with a nontrivial zero mass as
percentage effects \citep{chenroth2024}, which is a reason we lean on the sign and
the structural breaks rather than on the asinh coefficient's size (Table~\ref{tab:did}).
Identification requires parallel paths absent the shock
...
treatment year to 2023, moves $\gamma$ only modestly, to $2.71$; see below). The civilian
...
treatment date induces --- exactly the small-number-of-policy-changes setting
\citet{conleytaber2011} and \citet{fermanpinto2019} analyse, and our randomisation-inference
approach follows their prescription of resampling treatment assignment rather than trusting
asymptotic cluster-robust inference in this regime. We also report wild-cluster-bootstrap
$p$-values \citep{cameronbellermillergelbach2008} (few clusters,
...
externally compiled priority list, and a placebo. On the surge basket, exports to Russia rise by
...
$p = 0.016$). The externally compiled priority list, which is not selected on the Kazakh outcome, gives the
...
the placebo row has $N = 200$ (the 25 civilian
lines). The residual-list row is estimated on a purged sample, $N = 368$ (the 26 residual
lines plus the 20-line civilian control excluded of the surge basket itself; JIE round-2
review, Referee B N1 -- the full-panel control used in an earlier draft of this row
double-counted the 29 surge-basket lines as "control," which is corrected here). The surge
basket is selected on the
inbound (West + China) and outbound flows; the priority list (50 codes) is externally compiled.
Row 1 (surge basket) and row 2 (priority list) are estimated against the full 75-line panel;
row 3 (residual list) is estimated against the 46-line purged sample described above, so its
control pool differs from rows 1--2. No stars are reported in row 1: the surge-basket
coefficient's magnitude is not separable from the sample selection (Panel C), including for
the KZ-reported-imports column, whose apparent significance should be read against Panel C's
randomisation-inference null for that basket rather than taken at face value. The residual
row's star reflects the wild-cluster bootstrap $p$, the reliable statistic at 46 clusters.}
\label{tab:did}
\footnotesize
...
\quad\emph{of which, 26 lines, purged civilian control} & 1.94 (0.90)$^{\dagger}$ & 1.12 (0.83) & --- & --- \\
...
\midrule
\multicolumn{5}{@{}l}{\emph{Panel C. selection-rule-matched and random-basket randomisation inference, exports to Russia}} \\
\multicolumn{5}{@{}l}{\quad surge basket, rule-matched null $\gamma$: mean $2.79$, s.d.\ $1.50$; $P(|\gamma^{\text{perm}}|\ge 2.44) = 0.58$} \\
\multicolumn{5}{@{}l}{\quad surge basket, existence null: $P(\text{rule selects}\ge 29\text{ lines}\mid H_0) = 0.000$} \\
\multicolumn{5}{@{}l}{\quad priority list, random-50-line-basket null $\gamma$: mean $0.00$, s.d.\ $0.97$; $P(|\gamma^{\text{perm}}|\ge 2.88) = 0.002$} \\
\bottomrule
\end{tabular}

\vspace{2pt}
\footnotesize $^{*}$ $p<0.05$, $^{**}$ $p<0.01$, $^{***}$ $p<0.001$ (cluster-robust; before
multiple-testing adjustment); $^{\dagger}$ wild-cluster bootstrap $p<0.05$, the reliable
statistic at 46 clusters (analytic cluster-robust $p$ is unreliable at this cluster count).
After Holm adjustment across the four surge-basket outcomes,
exports to Russia and KZ-reported imports remain significant at 5\%. Surge-basket and
priority-list rows in Panel A have $N = 600$ (75 lines $\times$ 8 years); the residual-list row
has $N = 368$ (46 lines $\times$ 8 years, purged of the surge basket on both treatment and
control sides); the placebo row has $N = 200$. The residual-list row's wild cluster bootstrap
$p = 0.035$ outbound, $p = 0.238$ inbound. Panel B: exact PPML coefficients
$\beta = 1.26$ ($p = 0.02$) outbound and
$\beta = 0.85$ ($p < 10^{-4}$) inbound. Panel C: the surge-basket coefficient's \emph{magnitude}
is not separable from the post-outcome selection, but the \emph{existence} of a 29-line basket
is not a chance artefact under either a free or a trend-preserving permutation; the priority
list's coefficient is, by contrast, unusual relative to a random 50-line basket of any HS6
lines, which is consistent with it being carried substantially, though not entirely, by the
(genuinely treated) surge lines it contains: the residual 26 lines' own smaller, purged-control
DiD is independently significant on the outbound flow (row 3).
\end{table}

\paragraph{The reform confound.} Three things separate the reorientation from the
outward-oriented policy agenda of 2022. First, the civilian control basket behaves in a way a
broad reform cannot explain: on the annual data its inbound flow shows no 2022 movement at all, and
...
is much sharper still. Second, the outbound
...
2018--2021 mean (\$11m per year); because the pre-2022 outbound series is itself declining
...
inbound ($\approx 0.54$, on aggregate 2022--2025 minus baseline) and against West + China
($\approx 0.15$, ``one-sixth''); the latter is computed on 2022--2024 only, since the 2025
West + China figure in Table~\ref{tab:magnitudes} is Western-only (China not yet reported) and
mixing it into a West + China increment would understate it by construction; the
2022--2025 mixed-basis figure gives the same ratio to two decimal places. The
\emph{matched-cell} aggregate ratio
($\approx 0.11$, sum of re-export value over sum of inbound value across the 112 matched
HS6$\times$period cells) used in the weight-ratio discussion below is a third, separate
construction.} The inbound leg is the more weakly identified of
...
weak on both (slopes of $0.20$ and $0.48$, significant only in the annual f.o.b.\
specification).
...
flow-through are the same construction on different units, and here they coincide at $0.11$, so a
...
processing. If we restrict to the six near-pure-transit HS6$\times$period cells where value
flow-through is near one, the median weight ratio is $0.74$ ($0.26$ and $0.94$ at the
...
Let $m$ be the wholesale-and-freight margin Kazakhstan retains on a transiting good as a share of
...
about 49\%. A wholesale-and-freight margin sits in the lower part of that range. The
...
freight wedge gives the lower part of the Bureau of National Statistics bracket. The resources
table's trade-margin component alone (net of transport) is 48\% for machinery and electronics,
almost the whole 49\% ceiling, which confirms transport is a small share of the full margin for
this category but does not, on its own, pin a wholesale-only share: the table does not separate
wholesale from retail, so it cannot independently validate a band meant to exclude the retail
leg. We use \textbf{6--14\%} and treat it as a calibration, not an estimate, because no
line-by-line margin, and no wholesale/retail split, is identified in our data. Since
$\bar v^{TT} \approx \bar v^{M}$ the input--output
step does no work, so the headline is a direct function of $m$ and we carry it as a band. (We
do not use the matched-cell unit-value wedge to pin $m$ either: censoring negative-margin cells
inflates the gross margin to about 34\%,
...
retained. We do not fully reconcile the 34\% figure with our band: the under-recording bound we
can support elsewhere in the data (13--21\%, below) is not large enough on its own to explain
the gap, so part of the 34\% likely reflects the basis mismatch rather than under-invoicing
alone, but we cannot separate the two with the data at hand, and we treat 34\% as an upper
bound the paper's qualitative reading must survive, not a validated point estimate.)
...
Table~\ref{tab:appd}, gives the full sweep, the input--output cross-check and the matched-cell
wedge detail).
...
robust to any margin a wholesale-and-freight reading can support; it fails only if the corridor
...
for domestic value added to reach one-fifth \emph{of the gross rerouted flow} (a more
demanding benchmark than one-fifth of a produced dollar, which $m \approx 19\%$ already
reaches), the trade margin would have to be about $25\%$, which, holding the recorded import cost fixed,
...
macro level the gross reoriented flow, about \$0.6bn a year with the inbound and onward legs
combined, is 0.2--0.3\% of GDP; the 2022 current-account surplus, the reserve accumulation of
2023--2025 and the rise in services value added are an order of magnitude too large to be
driven by it.
...
four, with no post-2022 increase. Across the whole
...
\eqref{eq:build}'s threshold is on a sum, not a product, so no single term of \eqref{eq:gates}
is dispositive on its own; what follows states, for Kazakhstan, what each argument in
Sections~\ref{sec:shock}--\ref{sec:why} establishes on its own terms. The market-access gate
is open: under the Eurasian Economic Union the good moves to Russia with no transformation and
no border, so $V_T$ is large. Two pieces of evidence are suggestive, not dispositive
(Section~\ref{sec:framework}), that a domestic plant's transformation margin sits below the
thin re-export markup ($\mu_P \lesssim \mu_T$): the sector's $0.69$ value-added multiplier,
which bears on $\mu_P$ only loosely, and its near-zero domestic base, which is itself the
outcome the paper is trying to explain and so risks circularity as evidence for the margin
comparison. If that comparison holds, building is dominated in \eqref{eq:build} before the
option value is added; if it does not, the null is equally consistent with Kazakhstan simply
lacking a comparative advantage in the sector, customs union or not
(Section~\ref{sec:framework}). The re-export flow is not distinguishable in price or weight
from ordinary Kazakh trade (Section~\ref{sec:capture}). An open market-access gate is close to
sufficient for $R \approx 0$ only jointly with that margin condition, not on its own, which
limits what the case can say about the other two gates: we observe a single configuration,
with no gate clearly favourable and the three not separately identified. What we can add is a
state-fund pipeline that is inconsistent with the institutional gate as the binding constraint
for this null (the least
...
Kazakhstan's state
investment corporation (QIC, a subsidiary of the Baiterek group) faces neither an
external-finance nor an exit constraint, so the institutional gate does not bind for it the
way it binds for an outside investor. Its deployment in fact \emph{rose} sharply after 2022:
about \$2.2bn cumulative over 2015--2025, with about \$1.5bn of it deployed in 2025 alone, more
than twelve times the \$120m deployed in 2024, of which \$1.0bn was a single convertible-bond
investment in the steel producer Qarmet \citep{qicaifcifc2026pe}. But the report's named
QIC-linked investments are a minority stake in Almaty International Airport, a 160MW
power plant in Mangystau, a poultry producer, and the Qarmet bond: domestic infrastructure,
energy and food, none in a surge-basket line and none in corridor logistics built for the
flow. The report does note that manufacturing became a prominent sector in the market's
recorded deal activity from 2023 onward, but that finding covers the market as a whole
together with QIC, not these specific named investments. This is
...
in 2022--2025); the capacity evidence comes from plant announcements and public reporting. A
parallel announcement search for the same 2022--2025 window, using the same public-reporting
method, on electrical-machinery, component and instrument capacity in Kazakhstan turned up
no comparable project; general reporting on the sector describes market growth and import
volumes but no named new-capacity announcement in the reoriented lines
(Appendix~\ref{app:search}). We treat this as a
weaker check than the auto-sector search, which drew on more extensive public reporting on
that sector specifically, rather than as an equally thorough symmetric search, but it does not
overturn the null and is one more piece of evidence for it. Much of the auto capacity also
predates the reorientation
(Section~\ref{sec:investment}). The two shocks also differ along several axes at once:
...
is inconsistent with the institutional gate for the \emph{shock-specific} null, but there is one place in
...
private-equity market's exits run overwhelmingly through privately negotiated routes rather
than public markets: trade sale, buyback and secondary sale together account for about
three-quarters of surveyed exit preferences, against under a tenth for a public listing
...
it is the subject of a companion paper (available from the author).
...
not in the functional form of \eqref{eq:gates}, which we do not derive and do not treat as a
necessity result, but in the case-by-case arguments of Sections~\ref{sec:shock}--%
\ref{sec:why}: $R$ is plausibly higher when the market-access gate is closed to a pure
re-export and the irreversibility gate is open, and higher still when the institutional gate is
open as well. A country unfavourable on the first two is unlikely to show a response
irrespective of its institutions, and a country favourable on all three is a good candidate for
a supply response even with a shallow capital market.
...
shock, with surge-basket exports to Russia rising several-fold from 2022 (Armenia on the same
break timing as Kazakhstan; the Kyrgyz Republic elevated from 2022 on a noisier series --
Appendix~\ref{app:breaks}), and they share Kazakhstan's value on every moderator: the shock is event-driven,
its persistence uncertain \emph{ex ante}, policy risk is high, and the customs union removes
the need to transform. Two things are worth noting. Both neighbours' flows had already
...
but not for its persistence. We do not, however, have deal-level data for Armenia
...
The named population of post-2022 intermediaries is
\{Kazakhstan, Armenia, the Kyrgyz Republic, Belarus\} inside the Eurasian Economic Union and
\{Georgia, T\"urkiye\} outside it. The distinction matters for the framework: non-members
route goods to Russia across a customs border, so their market-access gate is not fully open,
and the framework predicts they are the more likely of the two groups to see a domestic
...
the Kyrgyz Republic. We cannot run the deal-database investment test for either: our
commercial extracts are Kazakhstan-only. As a lower-fidelity substitute we ran a public-record
announcement search, on the same 2022--2025 window and the same electrical-machinery,
component and instrument categories as the corridor's own surge basket, for new capacity in
T\"urkiye and Georgia (Appendix~\ref{app:search} gives the protocol); it turned up no comparable project in either country. This does not
confirm the framework's prediction that the non-members would be more likely to see a
processing or assembly response: a public-record search is a weak instrument for detecting a
marginal capacity change, especially in T\"urkiye's large pre-existing manufacturing base, so
absence of a reported project is not evidence of absence in the way the Kazakhstan-focused
commercial-database test is. The prediction remains untested at the standard the rest of the
paper holds itself to, and a proper test would need the same kind of greenfield-investment
register (fDi Markets or Orbis Cross-Border Investment) named in the Limitations below. The
external-validity claim we can support is therefore
...
Firm-level evidence makes the same point at smaller scale: demand shocks induce upgrading
when meeting the demand requires transformation, not when trade is a costless substitute.
A randomised expansion of foreign market access for Egyptian rug producers, who could not
resell into that demand without meeting buyers' quality specifications, raised
profits, productivity and product quality \citep{atkinkhandelwalosman2017}; the 1994 Mexican
peso devaluation raised the return to exporting for Mexican plants that could not serve the
newly cheaper-to-reach foreign market without upgrading quality and, with it, the skill mix
of their workforce \citep{verhoogen2008}. In both cases $V_T$ is effectively unavailable --
the demand cannot be captured by pure re-export or resale -- so the shock's effect passes
through the market-access gate into $\mu_P$ and $I$ rather than being absorbed as $V_T$, the
opposite of the Kazakhstan configuration.

We cannot, however, separate the irreversibility and institutional
gates across countries. Doing so would require regressing sector-level greenfield investment on
shock persistence and financial depth in a panel of intermediaries and contrast cases. In the sample available
...
electronics-adjacent chapters; we read them jointly, and use a simple combined score only to
order the candidates, not to rate them (Figure~\ref{fig:priority}).
...
and a credible listing-based exit, rather than relying only on the privately negotiated trade
sales, buybacks and secondary sales that dominate exit preferences today. That is a question
about capital-market institutions, taken up in a companion paper.
...
which is close to sufficient for a null only jointly with a small transformation margin, not
on its own (Section~\ref{sec:framework}). The corresponding lever is to make some
...
``Avoid'' rows of Table~\ref{tab:priority}, where domestic assembly of imported kits earns a
...
independent managers, with larger syndicated tickets and a credible public-market exit route:
a public listing draws under a tenth of surveyed exit preferences today, against the trade
sales, buybacks and secondary sales that together dominate \citep{qicaifcifc2026pe}, and
building the domestic private-equity exit market alongside it means widening that route.

Across all three levers the first call on investment is the logistics platform itself
(warehousing, transport support and Trans-Caspian capacity): as Section~\ref{sec:where} shows,
it carries the highest domestic value-added multiplier of the candidate sectors and creates
value that holds whether or not any particular flow persists.
...
one per cent of Kazakh GDP. The route matters as the marginal
...
route can actually take. It also lasts only as long as the chokepoint disruptions persist and
China's route diversification remains incomplete.

\subsection{China: lowering barriers to greater Kazakh value capture}
...
findings. Value-added-tax rebate and export-processing rules, major instruments of Chinese
...
without a second conformity process. Coordination of bonded-processing zones under the
...
precondition for private capital to act once those two have been pulled.
...
about a tenth of the manufacturing equivalent, customs duty under one per cent of revenue, a
fraction of one per cent of GDP, and no detectable investment response. The country is
...
when does it merely pass through? Production is a more plausible response the more that the
demand cannot be met by trade alone (a closed market-access gate), the more that the shock is
durable and certain enough, and the technology cheap enough to sink, that building beats
waiting (an open irreversibility gate), and the more that the capital market can fund the
projects that pass (an open institutional gate); we treat these as three considerations that
raise the odds of a supply response, not as conditions we derive to be jointly necessary. The
Kazakhstan case pins down the first
...
is inconsistent with the institutional gate as the binding constraint for this null (though its mandate
and its cross-border-compliance exposure cloud the comparison). Whether the irreversibility
gate \emph{independently} binds the case cannot say: because the re-export flow in fact
persisted through 2025, the null cannot be read as a response to an observed transitory shock;
what a 2022 entrant faced was uncertainty about the shock's duration, and the
...
open. Armenia and T\"urkiye show the same 2022 surge (the Kyrgyz Republic and Georgia are
elevated from 2022 on noisier series; Appendix~\ref{app:breaks}) and, on the evidence we have, no domestic
...
lines of this study, where no capacity was added. The mechanism, too, is the one the
...
gate plus directed institutions, exactly the configuration under which the framework predicts a
...
configuration, in which the three gates are not separately identified. It can still identify
the market-access gate as open and find evidence inconsistent with the capital-market
institutional gate as the
binding constraint for the shock-specific null; what it cannot establish is that
...
transaction, and they are Kazakhstan-only, so we cannot use them to test the framework's
prediction that non-EAEU intermediaries (T\"urkiye, Georgia) should be more likely to see a
processing response. A dedicated greenfield register (fDi Markets, or Orbis Cross-border
Investment) would close both gaps; the plant-level evidence we use in Section~\ref{sec:why},
and the public-record searches in Sections~\ref{sec:why} and~\ref{sec:moderators} for
components-sector capacity in Kazakhstan and for a differential response in T\"urkiye and
Georgia, are manual substitutes, weaker instruments than a purchased register. The two
searches are not evidence of the same kind: the Kazakhstan components search finds no
capacity where the framework predicts none, which is consistent with the framework; the
T\"urkiye/Georgia search finds no differential response where the framework predicts one
\emph{would} appear for non-EAEU intermediaries, which is a weak-instrument null that fails to
find the predicted difference and is therefore not supportive of the prediction, not
consistent with it. The cross-country prediction remains untested at the standard the rest of
the paper holds itself to.

Several measurement choices also bear on the estimates. The inbound side is partner-reported
(mirror) data, which overstate Kazakh absorption for onward-moving goods. The inbound flow is
...
post-2022 outcome; the externally compiled priority list breaks at the same date, and its
significant coefficient is carried substantially, though not entirely, by the 24 lines it
shares with the surge basket -- the 26 it does not share are, against a control purged of the
surge basket, independently significant on the outbound flow but at a smaller magnitude
(Table~\ref{tab:did}), so this is partial, smaller-scale corroboration
rather than a fully independent replication. We
therefore read the cross-sectional difference-in-differences alongside the structural breaks
and the neighbour comparison rather than on its own, and a placebo on the largest civilian lines
...
M\&A, Buyout\slash LBO, PE Growth\slash Expansion, all VC stages, Angel,
Accelerator\slash Incubator; Deal Date range as above.
...
\begin{table}[htbp]
\centering
\caption{Difference-in-differences robustness, surge basket, exports to Russia. The placebo's
fake treatment falls on the six largest civilian lines by pre-2022 size; none of the six is in
the surge basket.}
\label{tab:appc}
\footnotesize
\begin{tabular}{@{}lrrr@{}}
...
Priority list (50 HS6, externally compiled)             & 2.88 & 0.74 & $<0.001$ \\
\quad of which, 26 lines, vs.\ purged civilian control ($N=368$) & 1.94 & 0.90 & 0.037 \\
\quad\quad wild-cluster bootstrap $p$ (B $=$ 1999)      &      &      & 0.035 \\
\quad\quad pre-trend joint Wald ($F$, 3 and 45 DoF)     & 0.75 &      & 0.528 \\
2018 event-study coefficient (outside selection window) & 0.19 & 0.90 & 0.835 \\
...
\multicolumn{4}{@{}p{15.3cm}}{\footnotesize Free permutation (year labels within HS6, rule
...
1.50, 95th percentile 5.37, and $P(|\gamma^{\text{perm}}|\ge 2.44)=0.58$.} \\
\multicolumn{4}{@{}p{15.3cm}}{\footnotesize Trend-preserving cyclic-shift null: mean 5.2 lines
selected (max 19), $P(\ge 29\mid H_0)=0.000$; the cyclic-shift $\gamma$ null (which preserves
each line's own trend and autocorrelation, unlike the free permutation) has mean 1.64, s.d.\
1.20, and $P(|\gamma^{\text{cyc}}|\ge 2.44)=0.29$ --- narrower than the free-permutation null,
as expected, but not overturning the conclusion that the magnitude is not cleanly separable
from the selection.} \\
\multicolumn{4}{@{}p{15.3cm}}{\footnotesize Priority list: a random 50-line basket drawn from
all HS6 lines Kazakhstan exports to Russia has null $\gamma$ mean 0.00, s.d.\ 0.97, and
$P(|\gamma^{\text{perm}}|\ge 2.88)=0.002$ --- consistent with the priority-list coefficient
being carried by its genuinely-treated overlap with the surge basket (Table~\ref{tab:did}), not
itself an artefact of drawing 50 lines at random.} \\
\bottomrule
\end{tabular}
\end{table}
...
corridor never performs) about $49\%$; the trade-margin component alone (net of transport) is
$48\%$ for the same category, confirming that transport is a small part of the full margin
here, though the table does not separate wholesale from retail and so cannot independently
validate a wholesale-only reading. The 6--14\% band is a wholesale-and-freight reading, above
the floor and well below the ceiling; see below for how it relates to the $0.34$ matched-cell
estimate.
...
\vspace{2pt}
\footnotesize Anchors: transport-only $m\approx 0.01$; wholesale-and-freight band
...
We do not fully reconcile the censored matched-cell gross margin ($0.34$) with the $6$--$14\%$
band. The under-recording of outbound exports that would be needed to explain a one-fifth
domestic-value reading from the gross rerouted flow is $13$--$21\%$ (Section~\ref{sec:capture});
that bound is not large enough on its own to bring $0.34$ down into our band, so at least part
of the gap likely reflects the c.i.f./f.o.b.\ basis mismatch rather than under-invoicing alone.
We cannot separate the two components with the data at hand, and we treat $0.34$ as an upper
bound the paper's qualitative reading must survive (Table~\ref{tab:appd} shows it does, at
``about a third''), not as a validated point estimate of $m$.
...
Persistence, however, does not: T\"urkiye's flow peaks in 2023 at \$207m and recedes to \$73m
by 2025, a reversion pattern like Armenia's and the Kyrgyz Republic's rather than Kazakhstan's
sustained \$119--145m; Kazakhstan is the only intermediary in the table whose post-2022 level
does not recede toward its pre-2022 trajectory.
...
\toprule
& 2018 & 2019 & 2020 & 2021 & 2022 & 2023 & 2024 & 2025 & sup-$F$ & $p$ & jump yr \\
\midrule
Kazakhstan (EAEU)        & 17 & 12 & 7 & 8 & 128 & 145 & 119 & 133 & 150.7 & $<0.001$ & 2022 \\
Armenia (EAEU)           & 15 & 9 & 7 & 9 & 71 & 93 & 42 & 13 & 10.5 & 0.015 & 2022 \\
Kyrgyz Republic (EAEU)   & 1 & 3 & 6 & 7 & 12 & 30 & 12 & 12 & 13.4 & 0.004 & 2019 \\
T\"urkiye (non-EAEU)     & 53 & 55 & 47 & 58 & 119 & 207 & 98 & 73 & 11.9 & 0.007 & 2022 \\
Georgia (non-EAEU)       & 0.2 & 0.7 & 0.4 & 0.7 & 1.0 & 1.1 & 1.7 & 0.7 & 6.9 & 0.075 & 2019 \\
\bottomrule
\end{tabular}

\vspace{2pt}
\footnotesize Kazakhstan row from Table~\ref{tab:magnitudes}. sup-$F$ is a mean-shift test on
a persistent series; we read it as evidence of a break, not a cardinal magnitude, and it does
not itself date the break. ``Largest jump'' is the single year with the biggest
year-on-year change in $\text{asinh}$(exports); it is a noisy diagnostic at these small,
single-digit-to-low-double-digit levels, and for the Kyrgyz Republic and Georgia it lands on
2019 rather than 2022 despite both countries' 2022 levels also rising over their 2018--2021
average (JIE round-2 review, Referee B N3). We read the sup-$F$ rejection, not the single
largest jump, as the evidence of a common disruption; the ``same 2022 break'' language
elsewhere in the paper is best read as holding cleanly for Armenia and T\"urkiye, and as
``elevated from 2022'' rather than a dated break for the Kyrgyz Republic and Georgia,
whose series are shorter and noisier. We attempted
a Newey--West-corrected version of the sup-$F$ test for each country; on these 8-year annual series
the correction was not numerically stable (the bandwidth selection is itself unreliable at
this sample size), unlike the $\sim$70-observation monthly series in
Section~\ref{sec:shock}, where it is decisive and leaves the confidence interval essentially
unchanged.
\end{table}

\section{Public-record search protocols}\label{app:search}
\emph{For online publication.}

\medskip
Two public-record searches are referenced in Sections~\ref{sec:investment}
and~\ref{sec:moderators} as lower-fidelity substitutes for a purchased greenfield-investment
register (fDi Markets or Orbis Cross-Border Investment), which is unavailable to this project.
Both used the same method: general web search and news aggregation for named capacity
announcements (plant openings, assembly lines, production-capacity expansions) in the stated
sector and window, cross-checked against the deal databases of Section~\ref{sec:data} where a
transaction existed. Neither search used a purchased press-clipping service or a systematic
industry-directory sweep, so both are weaker instruments than the auto-sector triangulation
in Section~\ref{sec:investment}, which drew on more extensive public reporting specific to
that sector.

\begin{itemize}
\item \textbf{Kazakhstan components search} (Section~\ref{sec:investment}): window
2022--2025; sectors electrical machinery, components, and instruments (the surge-basket
adjacent categories); search conducted 2026-09 using general web search for named
new-capacity announcements. Result: no named new-capacity project found; general reporting on
the sector describes market growth and import volumes only.
\item \textbf{T\"urkiye/Georgia components search} (Section~\ref{sec:moderators}): same
window and sector definitions, applied to T\"urkiye and Georgia; search conducted 2026-09.
Result: no comparable project found in either country. As Section~\ref{sec:moderators}
states, this is read as an uninformative weak-instrument null for the framework's
cross-country prediction, not as a confirmation.
\end{itemize}

Neither search was pre-registered or independently replicated; both are disclosed as
best-effort substitutes, not as a systematic literature or press-record audit.
