...
retains from it, and to 2025. It shows that a large,
...
The three conditions can be summarised as a product. Three of the four observations above
identify a factor that, on its own, is enough to drive the investment response to zero: an
extreme irreversibility configuration, an open market-access gate (transformation not required,
so $V_T$ is large), or a capital market that cannot fund the projects that pass. Writing
\begin{equation}
\begin{aligned}
...
both on aggregate 2022--2025 minus baseline; and the \emph{matched-cell} aggregate ratio
...
freight wedge gives the lower part of the Bureau of National Statistics bracket. We use
\textbf{6--14\%} and treat it as a calibration, not an estimate, because no line-by-line
margin is identified in our data. Since $\bar v^{TT} \approx \bar v^{M}$ the input--output
step does no work, so the headline is a direct function of $m$ and we carry it as a band. (We
do not use the matched-cell unit-value wedge to pin $m$: censoring negative-margin cells
...
post-2022 outcome; the externally compiled priority list gives the same picture, but the
...
corridor never performs) about $49\%$; the 6--14\% band is a wholesale-and-freight
