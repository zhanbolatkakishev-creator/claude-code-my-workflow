...
$p = 0.035$; own level rise 2.4$\times$), though not on the inbound flow ($\gamma = 1.12$,
...
surge basket either: the residual lines move in the same direction at roughly a fifth of the
shared lines' scale. A placebo on the largest civilian lines runs the other way, with no
...
\quad\emph{Priority list (50 HS6), selection-free} & 82 & 89 & 59 & 61 & \textbf{306} & 293 & 229 & 187 \\
\bottomrule
\end{tabular}
...
Stars in row 1 are reported for completeness but should be read against Panel C: the
surge-basket coefficient's magnitude is not separable from the sample selection. The residual
...
symmetric announcement search for the same 2022--2025 window, using the same public-reporting
...
volumes but no named new-capacity announcement in the reoriented lines. We treat this as a
...
T\"urkiye and Georgia; it turned up no comparable project in either country. This does not
...
surge basket, independently significant on the outbound flow but at roughly a fifth of the
shared lines' magnitude (Table~\ref{tab:did}), so this is partial, smaller-scale corroboration
...
\toprule
& 2018 & 2019 & 2020 & 2021 & 2022 & 2023 & 2024 & 2025 & sup-$F$ & $p$ & largest jump \\
\midrule
