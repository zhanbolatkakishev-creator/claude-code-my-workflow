...
\citep{simola2024bofit,hilgenstock2023kse,kluge2024swp}.
...
available under academic licence, so the extract is replicable. QIC investment activity is
...
consistent with the inbound leg being the weaker of the two throughout.
...
not a chance artefact, $p < 0.001$), but the placebo $\gamma$ on those lines averages $2.8$,
so the coefficient's \emph{magnitude} is not separable from the selection. Second, the two
...
fixed---requires exports to Russia to be under-recorded by 15--26\%. Under-recording on that
...
in the series (Figure~\ref{fig:mismatch}). 2015 is a first-year zero as the databases ramp up
coverage; excluding it the pre-2022 rate is 8.5 per year, which would make the post period a
slight decline---so the ``no response'' reading is conservative on this margin. Dropping the
2022 trough, 2023--2025 runs at 9.0 per year (rate ratio 1.24, 95\% CI 0.74--2.01). The test
has 80\% power only against an increase of roughly 80\% or more, so it excludes a surge in
deal-making but not a modest step-up. Three features of the data say more than the count. First, reported deal \emph{value}
