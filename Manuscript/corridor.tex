...
supply response is trade rather than production. A corollary: if $\mu_P \le \mu_T$ (Kazakhstan
cannot even charge more for a transformed good than it pays for the input), building is
dominated before $\Omega$ is added. We do not identify $\mu_T$ from the unit-value
...
the 2022 movement at annual frequency; this is one more reason the reorientation is best seen
in the outbound series and the Western inbound. The sup-$F$ is computed without a HAC correction and
...
specification). Two features cut against domestic transformation, though neither is decisive.
First, the weight ratio (re-exported tonnage over imported tonnage in the matched cell) is
...
flow-through are the same statistic on different units and coincide here at $0.11$. A sub-one
weight ratio is therefore largely a flow-through fact, since only part of the imported tonnage
is re-exported to Russia at all, not evidence on weight gain per transiting unit. Eleven per
...
quartiles, on $n = 6$), showing no sign of local input added on the goods that do transit, but
on a thin slice of the basket. The
...
2022--2025 (a Poisson rate ratio of 0.96, 95\% CI 0.59--1.53); of the two soft years, 2015 (a
coverage ramp-up) and 2022 (the unrest), 2022 is the weakest in the series
...
assets, not new productive capacity: the ArcelorMittal Temirtau to Qarmet nationalisation
(2023), Eurocopter Kazakhstan and the Lokomotiv rolling-stock plant (re-nationalisations,
2024), and a \$0.8bn 2025 consolidation of rail-freight and forwarding assets. Second,
...
close to sufficient for $R \approx 0$ on its own, and it limits what the case can say about
the other two gates: we observe a single configuration, with no gate clearly favourable and
the three not separately identified. What we can add is a state-fund pipeline that
...
Third, the two shocks differ in persistence and certainty, in sunk-cost intensity, in whether
market access requires domestic content (EAEU local-content rules bind for vehicles, not for
the re-export, so the comparison also varies the market-access gate), and in the destination
market. The comparison is consistent with a durable, transformation-%
...
same timing, and share Kazakhstan's value on every moderator: the shock is event-driven, its
...
response to register. Country-level macro aggregates are in any case too noisy to isolate a
...
low multiplier of $0.69$ and forcing it would mostly deter the throughput) but the
...
the logistics platform itself (warehousing, transport support and Trans-Caspian capacity),
which has among the highest domestic value-added multipliers in the economy
...
value added a tenth of the manufacturing equivalent, is the smaller part of the cost. The larger part is dynamic. A rules-of-origin or local-content
