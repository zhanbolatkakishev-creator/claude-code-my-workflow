...
lines break discontinuously in May 2022 and rise roughly tenfold; the break date survives an
...
rather than the magnitude, so identification rests on the break dates, raw levels, and a
neighbour comparison rather than on the cross-sectional regression. Propagating a
...
each independently sufficient, though not shown to be independently necessary, to drive the
response toward zero. The case identifies an open market-access gate, created by a customs
union that makes trade a close substitute for production, and finds that the least
capital-constrained investor in the economy did not build either; whether irreversibility
independently binds cannot be settled here. Kazakhstan operates as a corridor, not a factory.
...
series, where a large pre-existing China flow dominates. Armenia and the Kyrgyz Republic show
the same 2022 break with no comparable domestic reform. A difference-in-differences against
...
same date, but we do not read it as independent corroboration of the basket's magnitude: its
own level rise is a third as large (3.5$\times$ against the surge basket's 11.6$\times$), and
the 26 priority-list lines that are not already in the surge basket show a
difference-in-differences indistinguishable from zero ($\gamma = -0.15$, bootstrap $p = 0.87$)
--- so the priority list's significant coefficient is carried entirely by the lines it shares
with the surge basket. A placebo on the largest civilian lines runs the other way, with no
...
production), which is close to sufficient for a null, and which also means Kazakhstan forgoes
...
$V_T$ to that channel, which is the addition the market-access gate below turns on. Four
observations follow.
...
constraint, did not build for the reallocation either, ruling out capital-market institutions
...
the residual-list row has $N = 208$ (26 lines); the placebo row has $N = 200$ (the 25 civilian
lines). The surge basket is selected on the
inbound (West + China) and outbound flows; the priority list (50 codes) is externally compiled.
Stars are reported for completeness but should be read against Panel C: the surge-basket
coefficient's magnitude is not separable from the sample selection, and the priority list's
significant coefficient is carried by the 24 lines it shares with the surge basket, not the 26
it does not (row 3).}
\label{tab:did}
\footnotesize
...
\quad\emph{of which, 26 lines not in the surge basket} & $-0.15$ (0.94) & $-0.49$ (1.00) & --- & --- \\
...
has $N = 208$; the placebo row has $N = 200$. The residual-list row's standard errors use a
wild cluster bootstrap ($p = 0.87$ outbound, $p = 0.67$ inbound), since the analytic
cluster-robust $p$ shown is unreliable with 26 clusters. Panel B: exact PPML coefficients
...
lines, which is consistent with it being carried by the (genuinely treated) surge lines it
contains rather than being itself a chance result.
\end{table}
...
Russia with no transformation and no border, so $V_T$ is large, and because a domestic plant's
...
the market-access gate as open and rule the capital-market institutional gate out as the
...
Georgia, are manual substitutes, weaker instruments than a purchased register, and all three
show no reorientation-linked capacity where the framework would predict one --- consistent
with, but not a confirmation of, the framework's cross-country prediction.
...
post-2022 outcome; the externally compiled priority list breaks at the same date, but its
significant coefficient is carried by the 24 lines it shares with the surge basket, not by the
26 it does not, so it is not independent corroboration of the cross-sectional magnitude. We
...
\quad of which, 26 lines not in the surge basket        & $-0.15$ & 0.94 & 0.873 \\
\quad\quad wild-cluster bootstrap $p$ (B $=$ 1999)      &      &      & 0.870 \\
...
a persistent series; we read it as evidence of a break, not a cardinal magnitude. We attempted
a Newey--West-corrected version of this test for each country; on these 8-year annual series
